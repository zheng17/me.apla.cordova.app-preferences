    ...
    [self.commandDelegate runInBackground:^{
        ...
        @try {
            ...
            if (settingsValue != nil) {
                ...
                if (returnVar == nil && target != nil){
//                    NSDictionary* targetDict= [target dictionaryRepresentation];
                    returnVar = [self findKey:settingsName inObject:target];
                }
...
- (id)findKey:(NSString *)keyname inObject:(id)object {
    ...
    for (id subKey in object) {
        ...
        if ([subKey containsString:keyname]) {
            return  subObject;
